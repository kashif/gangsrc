...
/*
  THIS CODE RUNS BETTER WHEN COMPILED **UNOPTIMIZED** (g++)
  (for epsilon-related stuff? or does non-optimizing set stuff to 0?)
*/
...
#pragma set woff 1209,1552
#endif

#define TRILIBRARY

static
void printf_( char const *format, ... )
{
#if DEBUG
  fprintf(stderr, "triangulator: ");
  va_list ap;
  va_start(ap, format);
...
{
  std::string s = "triangulator: error";
...
/*  triangles and shell edges are never represented by 0 pointers; they   */
...
/*  memory protection exception that would occur if 0 were dereferenced.  */
/*                                                                           */
...
REAL xmin=0, xmax=0, ymin=0, ymax=0;                              /* x and y bounds. */
REAL xminextreme=0;        /* Nonexistent x value used as a flag in sweepline. */
int inpoints=0;                                     /* Number of input points. */
int inelements=0;                                /* Number of input triangles. */
int insegments=0;                                 /* Number of input segments. */
int holes=0;                                         /* Number of input holes. */
int regions=0;                                     /* Number of input regions. */
long edges=0;                                       /* Number of output edges. */
int mesh_dim=0;                                  /* Dimension (ought to be 2). */
int nextras=0;                              /* Number of attributes per point. */
int eextras=0;                           /* Number of attributes per triangle. */
long hullsize=0;                            /* Number of edges of convex hull. */
int triwords=0;                                   /* Total words per triangle. */
int shwords=0;                                  /* Total words per shell edge. */
int pointmarkindex=0;             /* Index to find boundary marker of a point. */
int point2triindex=0;         /* Index to find a triangle adjacent to a point. */
int highorderindex=0;    /* Index to find extra nodes for high-order elements. */
int elemattribindex=0;              /* Index to find attributes of a triangle. */
int areaboundindex=0;               /* Index to find area bound of a triangle. */
int checksegments=0;           /* Are there segments in the triangulation yet? */
int readnodefile=0;                             /* Has a .node file been read? */
...
long incirclecount=0;                   /* Number of incircle tests performed. */
long counterclockcount=0;       /* Number of counterclockwise tests performed. */
long hyperbolacount=0;        /* Number of right-of-hyperbola tests performed. */
long circumcentercount=0;    /* Number of circumcenter calculations performed. */
long circletopcount=0;         /* Number of circle top calculations performed. */
...
int poly=0, refine=0, quality=0, vararea=0, fixedarea=0, regionattrib=0, convex=0;
int firstnumber=0;
int edgesout=0, voronoi=0, neighbors=0, geomview=0;
int nobound=0, nopolywritten=0, nonodewritten=0, noelewritten=0, noiterationnum=0;
int noholes=0;

int const TRIANGLE_NO_EXACT = 0;

int noexact=TRIANGLE_NO_EXACT;
int incremental=0, sweepline=0, dwyer=0;
int splitseg=0;
int docheck=0;
...
int order=0;
int nobisect=0;
int steiner=0, steinerleft=0;
REAL minangle=0, goodangle=0;
REAL maxarea=0;
...
triangle *dummytri = 0;
triangle *dummytribase = 0;      /* Keep base address so we can free() it later. */
...
shelle *dummysh = 0;
shelle *dummyshbase = 0;         /* Keep base address so we can free() it later. */
...
#define setvertices2null(triedge)                                             \
  (triedge).tri[3] = (triangle) 0;                                         \
  (triedge).tri[4] = (triangle) 0;                                         \
  (triedge).tri[5] = (triangle) 0;
...
#ifdef CDT_ONLY
#ifdef REDUCED
  printf_("triangle [-pAcevngBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
  printf_("triangle [-pAcevngBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
  printf_("triangle [-prq__a__AcevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  printf_("triangle [-prq__a__AcevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */

  printf_("    -p  Triangulates a Planar Straight Line Graph (.poly file).\n");
#ifndef CDT_ONLY
  printf_("    -r  Refines a previously generated mesh.\n");
  printf_(
    "    -q  Quality mesh generation.  A minimum angle may be specified.\n");
  printf_("    -a  Applies a maximum triangle area constraint.\n");
#endif /* not CDT_ONLY */
  printf_(
    "    -A  Applies attributes to identify elements in certain regions.\n");
  printf_("    -c  Encloses the convex hull with segments.\n");
  printf_("    -e  Generates an edge list.\n");
  printf_("    -v  Generates a Voronoi diagram.\n");
  printf_("    -n  Generates a list of triangle neighbors.\n");
  printf_("    -g  Generates an .off file for Geomview.\n");
  printf_("    -B  Suppresses output of boundary information.\n");
  printf_("    -P  Suppresses output of .poly file.\n");
  printf_("    -N  Suppresses output of .node file.\n");
  printf_("    -E  Suppresses output of .ele file.\n");
  printf_("    -I  Suppresses mesh iteration numbers.\n");
  printf_("    -O  Ignores holes in .poly file.\n");
  printf_("    -X  Suppresses use of exact arithmetic.\n");
  printf_("    -z  Numbers all items starting from zero (rather than one).\n");
  printf_("    -o2 Generates second-order subparametric elements.\n");
#ifndef CDT_ONLY
  printf_("    -Y  Suppresses boundary segment splitting.\n");
  printf_("    -S  Specifies maximum number of added Steiner points.\n");
#endif /* not CDT_ONLY */
#ifndef REDUCED
  printf_("    -i  Uses incremental method, rather than divide-and-conquer.\n");
  printf_("    -F  Uses Fortune's sweepline algorithm, rather than d-and-c.\n");
#endif /* not REDUCED */
  printf_("    -l  Uses vertical cuts only, rather than alternating cuts.\n");
#ifndef REDUCED
#ifndef CDT_ONLY
  printf_(
    "    -s  Force segments into mesh by splitting (instead of using CDT).\n");
#endif /* not CDT_ONLY */
  printf_("    -C  Check consistency of final mesh.\n");
#endif /* not REDUCED */
  printf_("    -Q  Quiet:  No terminal output except errors.\n");
  printf_("    -V  Verbose:  Detailed information on what I'm doing.\n");
  printf_("    -h  Help:  Detailed instructions for Triangle.\n");
  exit(0);
}
...
"Copyright 1996 Jonathan Richard Shewchuk  (bugs/comments to jrs@cs.cmu.edu)\n"
  ...
  printf_("School of Computer Science / Carnegie Mellon University\n");
  printf_("5000 Forbes Avenue / Pittsburgh, Pennsylvania  15213-3891\n");
  printf_(
"Created as part of the Archimedes project (tools for parallel FEM).\n");
  printf_(
"Supported in part by NSF Grant CMS-9318163 and an NSERC 1967 Scholarship.\n");
  printf_("There is no warranty whatsoever.  Use at your own risk.\n");
#ifdef SINGLE
  printf_("This executable is compiled for single precision arithmetic.\n\n\n");
#else /* not SINGLE */
  printf_("This executable is compiled for double precision arithmetic.\n\n\n");
#endif /* not SINGLE */
  printf_(
...
"input file will be read, and the Delaunay triangulation will be returned in\n"
  ...
  printf_(".node and .ele output files.  The command syntax is:\n\n");
#ifdef CDT_ONLY
#ifdef REDUCED
  printf_("triangle [-pAcevngBPNEIOXzo_lQVh] input_file\n\n");
#else /* not REDUCED */
...
"and possibly a .poly file and .area file as well.  The formats of these\n");
  printf_("files are described below.\n\n");
  printf_("Command Line Switches:\n\n");
  printf_(
...
"        and used to constrain edges in the mesh.  Further details on\n");
  printf_("        refinement are given below.\n");
  printf_(
...
"        with insufficient floating-point precision.  The specified angle\n");
  printf_("        may include a decimal point.\n");
  printf_(
...
"        assigned an attribute of zero.  The -A switch has an effect only\n");
  printf_("        when the -p switch is used and the -r switch is not.\n");
  printf_(
...
"    -v  Outputs the Voronoi diagram associated with the triangulation.\n");
  printf_("        Does not attempt to detect degeneracies.\n");
  printf_(
...
"    -g  Outputs the mesh to an Object File Format (.off) file, suitable for\n"
  ...
  printf_("        viewing with the Geometry Center's Geomview package.\n");
  printf_(
...
"        to impose segment constraints on later refinements of the mesh.\n");
  printf_("    -N  No output .node file.\n");
  printf_("    -E  No output .ele file.\n");
  printf_(
...
"        using a .node file for input, because no .node file will be\n");
  printf_("        written, so there will be no record of any added points.\n");
  printf_("    -O  No holes.  Ignores the holes in the .poly file.\n");
  printf_(
...
"        first point of the input .node or .poly file.  However, this switch\n"
  ...
  printf_("        is useful when calling Triangle from another program.\n");
  printf_(
...
"        triangulation.  Warning:  does not use exact arithmetic for all\n");
  printf_("        calculations.  An exact result is not guaranteed.\n");
  printf_(
...
"        is true to Ruppert's original algorithm, but can create needlessly\n"
  ...
  printf_("        small triangles near external small features.\n");
  printf_(
...
"        Triangle will run much more slowly.  `-VVV' gives information only\n"
  ...
  printf_("        a debugger could love.\n");
  printf_("    -h  Help:  Displays these instructions.\n");
  printf_("\n");
  printf_("Definitions:\n");
  printf_("\n");
  printf_(
...
"  falls in the interior of the circumcircle (circle that passes through all\n"
  ...
  printf_("  three vertices) of any triangle in the triangulation.\n\n");
  printf_(
...
"  to any other input point.  (The Voronoi diagram is the geometric dual of\n"
  ...
  printf_("  the Delaunay triangulation.)\n\n");
  printf_(
...
"  triangulation.  (A constrained Delaunay triangulation is not truly a\n");
  printf_("  Delaunay triangulation.)\n\n");
  printf_(
...
"  points are necessary to allow the segments to exist in the mesh while\n");
  printf_("  maintaining the Delaunay property.\n\n");
  printf_("File Formats:\n\n");
  printf_(
...
"  program, use the -z switch if you wish to number objects from zero.)\n");
  printf_("  Examples of these file formats are given below.\n\n");
  printf_("  .node files:\n");
  printf_(
...
"    -q, or -a is selected, each new Steiner point added to the mesh will\n");
  printf_("    have attributes assigned to it by linear interpolation.\n\n");
  printf_(
...
"    file produced by Triangle will contain boundary markers in the last\n");
  printf_("    column unless they are suppressed by the -B switch.\n\n");
  printf_("  .ele files:\n");
  printf_(
...
"    fourth, fifth, and sixth points lie on the midpoints of the edges\n");
  printf_("    opposite the first, second, and third corners.\n\n");
  printf_("  .poly files:\n");
  printf_(
"    First line:  <# of points> <dimension (must be 2)> <# of attributes>\n");
  printf_(
...
"    Following lines:  <point #> <x> <y> [attributes] [boundary marker]\n");
  printf_("    One line:  <# of segments> <# of boundary markers (0 or 1)>\n");
  printf_(
"    Following lines:  <segment #> <endpoint> <endpoint> [boundary marker]\n");
  printf_("    One line:  <# of holes>\n");
  printf_("    Following lines:  <hole #> <x> <y>\n");
  printf_(
...
"    of the .poly file with the extension .node; all sections of the file\n");
  printf_("    but the first are ignored.)\n\n");
  printf_(
...
"    triangulation, in which segments may be subdivided into smaller edges.\n"
  ...
  printf_("    Each segment, like each point, may have a boundary marker.\n\n");
  printf_(
...
"    is also eaten.  Do not place a hole directly on a segment; if you do,\n");
  printf_("    Triangle will choose one side of the segment arbitrarily.\n\n");
  printf_(
...
"    simultaneously and wish to assign an attribute to some region without\n");
  printf_("    imposing an area constraint, use a negative maximum area.\n\n");
  printf_(
...
"    use the -c switch, you can still produce concavities by appropriate\n");
  printf_("    placement of holes just inside the convex hull.\n\n");
  printf_(
...
"    line, and you'll have a needle-sharp triangle in your output - or a lot\n"
  ...
  printf_("    of tiny triangles if you're generating a quality mesh.\n\n");
  printf_(
...
"    plan to refine the output mesh, and don't want segments to be missing\n");
  printf_("    in later triangulations.\n\n");
  printf_("  .area files:\n");
  printf_("    First line:  <# of triangles>\n");
  printf_("    Following lines:  <triangle #> <maximum area>\n\n");
  printf_(
...
"    may be left unconstrained by assigning it a negative maximum area.\n");
  printf_("\n");
  printf_("  .edge files:\n");
  printf_("    First line:  <# of edges> <# of boundary markers (0 or 1)>\n");
  printf_(
...
"    infinite ray with only one endpoint.  For these edges, a different\n");
  printf_("    format is used:\n\n");
  printf_("        <edge #> <endpoint> -1 <direction x> <direction y>\n\n");
  printf_(
"    The `direction' is a floating-point vector that indicates the direction\n"
);
  printf_("    of the infinite ray.\n\n");
  printf_("  .neigh files:\n");
  printf_(
...
"    The first neighbor of triangle i is opposite the first corner of\n");
  printf_("    triangle i, and so on.\n\n");
  printf_("Boundary Markers:\n\n");
  printf_(
...
"  element mesh.  You can prevent boundary markers from being written into\n");
  printf_("  files produced by Triangle by using the -B switch.\n\n");
  printf_(
"  The boundary marker associated with each segment in an output .poly file\n"
);
  printf_("  or edge in an output .edge file is chosen as follows:\n");
  printf_(
...
"      (including boundaries of holes), then the edge is assigned the marker\n"
  ...
  printf_("      one (1).\n");
  printf_("    - Otherwise, the edge is assigned the marker zero (0).\n");
  printf_(
"  The boundary marker associated with each point in an output .node file is\n"
);
  printf_("  chosen as follows:\n");
  printf_(
...
"      is assigned the same marker.  If the point lies on several such\n");
  printf_("      segments, one of the markers is chosen arbitrarily.\n");
  printf_(
"    - Otherwise, if the point occurs on a boundary of the triangulation,\n");
  printf_("      then the point is assigned the marker one (1).\n");
  printf_("    - Otherwise, the point is assigned the marker zero (0).\n");
  printf_("\n");
  printf_(
...
"  all) in your input files.  Alternatively, you can mark some of them and\n");
  printf_("  leave others marked zero, allowing Triangle to label them.\n\n");
  printf_("Triangulation Iteration Numbers:\n\n");
  printf_(
...
"  triangulation, and output the files mesh.4.node, mesh.4.ele, and\n");
  printf_("  mesh.4.poly.  Files with no iteration number are treated as if\n");
  printf_(
...
"  from being overwritten.  (If the input is a .poly file that contains its\n"
  ...
  printf_("  own points, a .node file will be written.)\n\n");
  printf_("Examples of How to Use Triangle:\n\n");
  printf_(
...
"  object.1.node, if the points are omitted from object.1.poly) and write\n");
  printf_("  their constrained Delaunay triangulation to object.2.node and\n");
  printf_(
"  object.2.ele.  The segments will be copied to object.2.poly, and all\n");
  printf_("  edges will be written to object.2.edge.\n\n");
  printf_(
...
"  been broken up into multiple edges; the resulting constrained edges are\n");
  printf_("  written to object.1.poly.\n\n");
  printf_(
...
"    # A box with eight points in 2D, no attributes, one boundary marker.\n");
  printf_("    8 2 0 1\n");
  printf_("    # Outer box has these vertices:\n");
  printf_("     1   0 0   0\n");
  printf_("     2   0 3   0\n");
  printf_("     3   3 0   0\n");
  printf_("     4   3 3   33     # A special marker for this point.\n");
  printf_("    # Inner square has these vertices:\n");
  printf_("     5   1 1   0\n");
  printf_("     6   1 2   0\n");
  printf_("     7   2 1   0\n");
  printf_("     8   2 2   0\n");
  printf_("    # Five segments with boundary markers.\n");
  printf_("    5 1\n");
  printf_("     1   1 2   5      # Left side of outer box.\n");
  printf_("     2   5 7   0      # Segments 2 through 5 enclose the hole.\n");
  printf_("     3   7 8   0\n");
  printf_("     4   8 6   10\n");
  printf_("     5   6 5   0\n");
  printf_("    # One hole in the middle of the inner square.\n");
  printf_("    1\n");
  printf_("     1   1.5 1.5\n\n");
  printf_(
...
"  points but 4 have been marked to indicate that they lie on a boundary.\n");
  ...
  printf_("      11    3   1.5    1\n");
  printf_("      12    1.5   3    1\n");
  printf_("    # Generated by triangle -pqc box.poly\n\n");
  printf_("  Here is the output file `box.1.ele', with twelve triangles.\n\n");
  printf_("    12  3  0\n");
  printf_("       1     5   6   9\n");
  printf_("       2    10   3   7\n");
  printf_("       3     6   8  12\n");
...
"  added points.  Note also that <# of points> is set to zero to indicate\n");
  printf_("  that the points should be read from the .node file.\n\n");
  printf_("    0  2  0  1\n");
  printf_("    12  1\n");
  printf_("       1     1   9     5\n");
  printf_("       2     5   7     1\n");
  ...
  printf_("       1   1.5 1.5\n");
  printf_("    # Generated by triangle -pqc box.poly\n\n");
  printf_("Refinement and Area Constraints:\n\n");
  printf_(
...
"  is refined, so if there are large variations in area constraint between\n");
  printf_("  adjacent triangles, you may not get the results you want.\n\n");
  printf_(
...
"  hierarchical property applies only to the nodes at the corners of an\n");
  printf_("  element; other nodes may not be present in the refined mesh.\n\n");
  printf_(
...
"  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,\n");
  printf_("  suitable for multigrid.\n\n");
  printf_("Convex Hulls and Mesh Boundaries:\n\n");
  printf_(
...
"  -p switch), Triangle produces a list of its boundary edges (including\n");
  printf_("  hole boundaries) as a by-product if you use the -c switch.\n\n");
  printf_("Voronoi Diagrams:\n\n");
  printf_(
...
"  rays.  (The choice of filenames makes it easy to run the set of Voronoi\n");
  printf_("  vertices through Triangle, if so desired.)\n\n");
  printf_(
...
"  or if the triangulation has holes, the corresponding Voronoi diagram is\n");
  printf_("  likely to have crossing edges and unlikely to make sense.\n\n");
  printf_("Mesh Topology:\n\n");
  printf_(
...
"  output files with the recollection that the Delaunay triangulation and\n");
  printf_("  the Voronoi diagrams are planar duals.\n\n");
  printf_(
...
"  vertex j of the corresponding .v.node file; and Voronoi region k is the\n");
  printf_("  dual of point k of the corresponding .node file.\n\n");
  printf_(
...
"  of the Voronoi edge, respectively.  To find which Voronoi regions are\n");
  printf_("  adjacent to each other, just read the list of Delaunay edges.\n");
  printf_("\n");
  printf_("Statistics:\n");
  printf_("\n");
  printf_(
...
"  (or -rpNEP if you've got a .poly file for the existing mesh) will\n");
  printf_("  regenerate these statistics without writing any output.\n\n");
  printf_(
...
"  registers, then your output is guaranteed to be an absolutely true\n");
  printf_("  Delaunay or conforming Delaunay triangulation, roundoff error\n");
  printf_(
...
"  difference could be a factor of two.  These are precisely the inputs most\n"
  ...
  printf_("  likely to cause errors if you use the -X switch.\n\n");
  printf_(
...
"  silently prevent the tests from being performed exactly, while overflow\n");
  printf_("  will typically cause a floating exception.\n\n");
  printf_("Calling Triangle from Another Program:\n\n");
  printf_("  Read the file triangle.h for details.\n\n");
  printf_("Troubleshooting:\n\n");
  printf_("  Please read this section before mailing me bugs.\n\n");
  printf_("  `My output mesh has no triangles!'\n\n");
  printf_(
...
"    collinear, in which case you can hardly expect to triangulate them.\n");
  printf_("\n");
  printf_("  `Triangle doesn't terminate, or just crashes.'\n");
  printf_("\n");
  printf_(
...
"    coordinates all fall between 6247133 and 6247134, you're not leaving\n");
  printf_("    much floating-point precision for Triangle to work with.\n\n");
  printf_(
...
"    segments; zoom in closely, as such segments might look like a single\n");
  printf_("    segment from a distance.\n\n");
  printf_(
...
"    above.)  Again, I recommend compiling Triangle for double (rather\n");
  printf_("    than single) precision arithmetic.\n\n");
  printf_(
...
"    You may have eaten some of your input points with a hole, or by placing\n"
  ...
  printf_("    them outside the area enclosed by segments.\n\n");
  printf_(
...
"    point roundoff error.  Although these errors are rare, don't use the -X\n"
  ...
  printf_("    switch.  If you still have problems, please report the bug.\n");
  printf_("\n");
  printf_(
...
"  you'll have to connect each adjacent pair of points with a separate\n");
  printf_("  segment.  If you don't like it, tough.\n\n");
  printf_(
...
"  correctly for Triangle?  Are the triangles all listed so the first three\n"
  ...
  printf_("  points are their corners in counterclockwise order?\n\n");
  printf_("Show Me:\n\n");
  printf_(
...
"  Windows system.  If you didn't receive Show Me with Triangle, complain to\n"
  ...
  printf_("  whomever you obtained Triangle from, then send me mail.\n\n");
  printf_("Triangle on the Web:\n\n");
  printf_(
"  To see an illustrated, updated version of these instructions, check out\n");
  printf_("\n");
  printf_("    http://www.cs.cmu.edu/~quake/triangle.html\n");
  printf_("\n");
  printf_("A Brief Plea:\n");
  printf_("\n");
  printf_(
...
"  If you use a mesh generated by Triangle in a publication, please include\n"
  ...
  printf_("  an acknowledgment as well.\n\n");
  printf_("Research credit:\n\n");
  printf_(
...
"  J. Van Wyk, David F. Watson, and Binhai Zhu.  See the comments at the\n");
  printf_("  beginning of the source code for references.\n\n");
  exit(0);
}
...
/*****************************************************************************/
/*                                                                           */
/*  internal_error()   Ask the user to send me the defective product.  Exit.  */
/*                                                                           */
/*****************************************************************************/
...
#define STARTINDEX 1
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            minangle = (REAL) strtod(workstring, (char **) 0);
	  } else {
            minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            maxarea = (REAL) strtod(workstring, (char **) 0);
            if (maxarea <= 0.0) {
  ...
  if (noiterationnum) {
    ...
    sprintf_(outnodefilename, workstring, meshnumber + 1);
  ...
  )
{
  ...
  printf_("triangle x%lx with orientation %d:\n", (unsigned long) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == dummytri) {
    printf_("    [0] = Outer space\n");
  } else {
  ...
  if (printtri.tri == dummytri) {
    printf_("    [2] = Outer space\n");
  } else {
    printf_("    [2] = x%lx  %d\n", (unsigned long) printtri.tri,
           printtri.orient);
  }
  org(*t, printpoint);
  if (printpoint == (point) 0)
    printf_("    Origin[%d] = 0\n", (t->orient + 1) % 3 + 3);
  else
    printf_("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
  ...
  )
{
  ...
  printf_("shell edge x%lx with orientation %d and mark %d:\n",
  ...
  )
{
  ...
  if (sizeof(VOID *) > (uint)pool->alignbytes) {
  ...
  pool->firstblock = (VOID **) _malloc(pool->itemsperblock * pool->itembytes
                                      + sizeof(VOID *) + pool->alignbytes);
  if (pool->firstblock == (VOID **) 0) {
    error( "Out of memory.\n");
  }
  /* Set the next block pointer to 0. */
  *(pool->firstblock) = (VOID *) 0;
  poolrestart(pool);
}
  ...
  )
{
  ...
  if (pool->deaditemstack != (VOID *) 0) {
    ...
    if (pool->unallocateditems == 0) {
      ...
      if (*(pool->nowblock) == (VOID *) 0) {
        /* Allocate a new block of items, pointed to by the previous block. */
        newblock = (VOID **) _malloc(pool->itemsperblock * pool->itembytes
                                    + sizeof(VOID *) + pool->alignbytes);
        if (newblock == (VOID **) 0) {
          error( "Out of memory.\n");
        }
        *(pool->nowblock) = (VOID *) newblock;
        /* The next block pointer is 0. */
        *newblock = (VOID *) 0;
      }
  ...
  pool->items++;
  memset( newitem, 0, pool->itembytes );
  return newitem;
}
  ...
  )
{
  ...
  dummytribase = (triangle *) _malloc(triwords * sizeof(triangle)
                                     + triangles.alignbytes);
  if (dummytribase == (triangle *) 0) {
    error( "Out of memory.\n");
  }
  ...
  /* Three 0 vertex points. */
  dummytri[3] = (triangle) 0;
  dummytri[4] = (triangle) 0;
  dummytri[5] = (triangle) 0;
  ...
  if (useshelles) {
    ...
    /*   edge.                                                               */
    dummyshbase = (shelle *) _malloc(shwords * sizeof(shelle)
                                    + shelles.alignbytes);
    if (dummyshbase == (shelle *) 0) {
      error( "Out of memory.\n");
    }
...
void initializetrisegpools()
{
  ...
  if ((voronoi || neighbors) &&
      ( (uint)trisize < 6 * sizeof(triangle) + sizeof(int))) {
...
{
  /* Set triangle's vertices to 0.  This makes it possible to        */
  /*   detect dead triangles when traversing the list of all triangles. */
  dyingtriangle[3] = (triangle) 0;
  dyingtriangle[4] = (triangle) 0;
  dyingtriangle[5] = (triangle) 0;
...
triangle *triangletraverse()
{
  ...
  do {
    ...
    if (newtriangle == (triangle *) 0) {
      return (triangle *) 0;
    }
  } while (newtriangle[3] == (triangle) 0);            /* Skip dead ones. */
  return newtriangle;
}
...
{
  /* Set shell edge's vertices to 0.  This makes it possible to */
  /*   detect dead shells when traversing the list of all shells.  */
  dyingshelle[2] = (shelle) 0;
  dyingshelle[3] = (shelle) 0;
...
shelle *shelletraverse()
{
  ...
  do {
    ...
    if (newshelle == (shelle *) 0) {
      return (shelle *) 0;
    }
  } while (newshelle[2] == (shelle) 0);                /* Skip dead ones. */
  return newshelle;
}
...
{
  // bug trap -- hack to catch bug in software
  int unsigned const MAX_ITERATIONS = 1000;
  int unsigned i = 0;
  ...
  do {
    ...
    if (newseg == (struct edge *) 0)
    {
      return (struct edge *) 0;
    }
    if (i >= MAX_ITERATIONS)
    {
      // memory leak
...
void triangledeinit()
{
  ...
  free(dummytribase);
  dummytribase = (triangle *)0;
  if (useshelles) {
    pooldeinit(&shelles);
    free(dummyshbase);
    dummyshbase = (shelle *)0;
  }
  pooldeinit(&points);
#ifndef CDT_ONLY
  if (quality) {
  ...
  )
{
  ...
  /* Three 0 vertex points. */
  newtriedge->tri[3] = (triangle) 0;
  newtriedge->tri[4] = (triangle) 0;
  newtriedge->tri[5] = (triangle) 0;
  ...
  )
{
  ...
  /* Two 0 vertex points. */
  newedge->sh[2] = (shelle) 0;
  newedge->sh[3] = (shelle) 0;
  /* Initialize the two adjoining triangles to be "outer space". */
...
{
  /*
    THIS ROUTINE DOES NOT WORK FOR g++ -O because it optimizes
    away adding then subtracting 1.0
  */
#if 0
  REAL half = 0.0;
  REAL check = 0.0, lastcheck = 0.0;
  int every_other = 0;
  ...
  splitter += REAL(1.0);
  if (verbose > 1) {
    printf_("Floating point roundoff is of magnitude %.17g\n", epsilon);
    printf_("Floating point splitter is %.17g\n", splitter);
  }
#endif

#if 1
  epsilon = 0.5*DBL_EPSILON;
  splitter = 1.34217729e+08;
#endif
...
void triangleinit()
{
  ...
  recenttri.tri = (triangle *) 0;    /* No triangle has been visited yet. */
...
void checkmesh()
{
  ...
  noexact = 0;
  if (!quiet) {
    printf_("  Checking consistency of mesh...\n");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&triangles);
  triangleloop.tri = triangletraverse();
  while (triangleloop.tri != (triangle *) 0) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (oppotri.tri != dummytri) {
        ...
        if ((triangleloop.tri != oppooppotri.tri)
          ...
          printf_("  !! !! Asymmetric triangle-triangle bond:\n");
          if (triangleloop.tri == oppooppotri.tri) {
            printf_("   (Right triangle, wrong orientation)\n");
          }
          printf_("    First ");
          printtriangle(&triangleloop);
          printf_("    Second (nonreciprocating) ");
        ...
        if ((triorg != oppodest) || (tridest != oppoorg)) {
          printf_("  !! !! Mismatched edge coordinates between two triangles:\n"
                 );
          printf_("    First mismatched ");
          printtriangle(&triangleloop);
          printf_("    Second mismatched ");
          printtriangle(&oppotri);
          horrors++;
        }
      }
  ...
  if (horrors == 0) {
    if (!quiet) {
      printf_("  In my studied opinion, the mesh appears to be consistent.\n");
    }
  } else if (horrors == 1) {
    printf_("  !! !! !! !! Precisely one festering wound discovered.\n");
  } else {
    printf_("  !! !! !! !! %d abominations witnessed.\n", horrors);
  }
...
void checkdelaunay()
{
  ...
  noexact = 0;
  if (!quiet) {
    printf_("  Checking Delaunay property of mesh...\n");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&triangles);
  ...
  while (triangleloop.tri != (triangle *) 0) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      shouldbedelaunay = (oppotri.tri != dummytri)
            && (triapex != (point) 0) && (oppoapex != (point) 0)
      ...
      if (shouldbedelaunay) {
        if (incircle(triorg, tridest, triapex, oppoapex) > 0.0) {
          printf_("  !! !! Non-Delaunay pair of triangles:\n");
          printf_("    First non-Delaunay ");
          printtriangle(&triangleloop);
          printf_("    Second non-Delaunay ");
          printtriangle(&oppotri);
          horrors++;
        }
      }
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay.\n");
    ...
    printf_("  !! !! !! !! %d obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  noexact = saveexact;
}
  ...
  )
{
  ...
  if (verbose > 2) {
    printf_("  Queueing bad triangle:\n");
    printf_("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", insorg[0],
  ...
  newface->nextface = (struct badface *) 0;
  ...
  *queuetail[queuenumber] = newface;
  /* Maintain a pointer to the 0 pointer at the end of the queue. */
...
struct badface *dequeuebadtri()
{
  ...
  for (queuenumber = 63; queuenumber >= 0; queuenumber--) {
    ...
    if (result != (struct badface *) 0) {
      /* Remove the triangle from the queue. */
      queuefront[queuenumber] = result->nextface;
      /* Maintain a pointer to the 0 pointer at the end of the queue. */
      if (queuefront[queuenumber] == (struct badface *) 0) {
        queuetail[queuenumber] = &queuefront[queuenumber];
      }
      return result;
    }
  }
  return (struct badface *) 0;
}
  ...
  )
{
  ...
  if (addtolist && (!nobisect || ((nobisect == 1) && (sides == 2)))) {
    if (verbose > 2) {
      printf_("  Queueing encroached segment (%.12g, %.12g) (%.12g, %.12g).\n",
...
void makepointmap()
{
  ...
  if (verbose) {
    printf_("    Constructing mapping from points to triangles.\n");
  }
  traversalinit(&triangles);
  triangleloop.tri = triangletraverse();
  while (triangleloop.tri != (triangle *) 0) {
  ...
  )
{
  ...
  if (verbose > 2) {
    printf_("  Searching for point (%.12g, %.12g).\n",
  ...
  int unsigned MAX_ITERATIONS = 1000;
  int unsigned i = 0;

  while (1) {
    if (verbose > 2) {
      printf_("    At (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
...
{
  VOID **sampleblock = (VOID **)0;
  triangle *firsttri = (triangle *)0;
  ...
  long triblocks;
  long i, j;
  triangle ptr = (REAL **)0;   /* Temporary variable used by sym(). */


  if (verbose > 2) {
    printf_("  Randomly sampling for a triangle near point (%.12g, %.12g).\n",
           searchpoint[0], searchpoint[1]);
  }
  ...
  // added by N. Schmitt 11 Dec 2001
  if (torg == (REAL *)0) {
    internal_error();
  }
  ...
  if (verbose > 2) {
    printf_("    Boundary triangle has origin (%.12g, %.12g).\n",
           torg[0], torg[1]);
  }

  /* If a recently encountered triangle has been recorded and has not been */
  /*   deallocated, test it as a good starting point.                      */
  if (recenttri.tri != (triangle *) 0) {
    if (recenttri.tri[3] != (triangle) 0) {
      org(recenttri, torg);
      ...
      if (dist < searchdist) {
        ...
        searchdist = dist;
        if (verbose > 2) {
          printf_("    Choosing recent triangle with origin (%.12g, %.12g).\n",
                 torg[0], torg[1]);
        }
      }
    }
    ...
    for (j = 0; j < samplesperblock; j++) {
      ...
      if (sampletri.tri[3] != (triangle) 0) {
        ...
        if (dist < searchdist) {
          ...
          searchdist = dist;
          if (verbose > 2) {
            printf_("    Choosing triangle with origin (%.12g, %.12g).\n",
                   torg[0], torg[1]);
          }
        }
      }
  ...
  )
{
...
#ifdef SELF_CHECK
  if (top.tri == dummytri) {
    printf_("Internal error in flip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (checksegments) {
    ...
    if (toplshelle.sh != dummysh) {
      printf_("Internal error in flip():  Attempt to flip a segment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (verbose > 2) {
    printf_("  Edge flip results in left ");
    lnextself(topleft);
...
/*                 property.                                                 */
/*                                                                           */
/*  The point `insertpoint' is located.  If `searchtri.tri' is not 0,     */
/*  the search for the containing triangle begins from `searchtri'.  If      */
/*  `searchtri.tri' is 0, a full point location procedure is called.      */
...
/*  existing vertex.                                                         */
/*                                                                           */
/*  Normally, the parameter `splitedge' is set to 0, implying that no     */
  ...
  )
{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    if (checksegments && (splitedge == (struct edge *) 0)) {
    ...
    if (counterclockwise(rightpoint, leftpoint, botpoint) < 0.0) {
      printf_("Internal error in insertsite():\n");
      printf_("  Clockwise triangle prior to edge point insertion (bottom).\n");
    }
    if (mirrorflag) {
      if (counterclockwise(leftpoint, rightpoint, toppoint) < 0.0) {
        printf_("Internal error in insertsite():\n");
        printf_("  Clockwise triangle prior to edge point insertion (top).\n");
      }
      if (counterclockwise(rightpoint, toppoint, insertpoint) < 0.0) {
        printf_("Internal error in insertsite():\n");
        printf_("  Clockwise triangle after edge point insertion (top right).\n"
               );
      }
      if (counterclockwise(toppoint, leftpoint, insertpoint) < 0.0) {
        printf_("Internal error in insertsite():\n");
        printf_("  Clockwise triangle after edge point insertion (top left).\n"
               );
      }
    }
    if (counterclockwise(leftpoint, botpoint, insertpoint) < 0.0) {
      printf_("Internal error in insertsite():\n");
      printf_("  Clockwise triangle after edge point insertion (bottom left).\n"
             );
    }
...
#endif /* SELF_CHECK */
    if (verbose > 2) {
      printf_("  Updating bottom left ");
      printtriangle(&botright);
      if (mirrorflag) {
        printf_("  Updating top left ");
        printtriangle(&topright);
        printf_("  Creating top right ");
        printtriangle(&newtopright);
      }
      printf_("  Creating bottom right ");
    ...
    if (counterclockwise(rightpoint, leftpoint, botpoint) < 0.0) {
      printf_("Internal error in insertsite():\n");
      printf_("  Clockwise triangle prior to point insertion.\n");
    }
    if (counterclockwise(rightpoint, leftpoint, insertpoint) < 0.0) {
      printf_("Internal error in insertsite():\n");
      printf_("  Clockwise triangle after point insertion (top).\n");
    }
    if (counterclockwise(leftpoint, botpoint, insertpoint) < 0.0) {
      printf_("Internal error in insertsite():\n");
      printf_("  Clockwise triangle after point insertion (left).\n");
    }
    if (counterclockwise(botpoint, rightpoint, insertpoint) < 0.0) {
      printf_("Internal error in insertsite():\n");
      printf_("  Clockwise triangle after point insertion (right).\n");
    }
#endif /* SELF_CHECK */
    if (verbose > 2) {
      printf_("  Updating top ");
      printtriangle(&horiz);
      printf_("  Creating left ");
      printtriangle(&newbotleft);
      printf_("  Creating right ");
      printtriangle(&newbotright);
    }
  }
...
#ifdef SELF_CHECK
          if (insertpoint != (point) 0) {
            if (counterclockwise(leftpoint, insertpoint, rightpoint) < 0.0) {
              printf_("Internal error in insertsite():\n");
              printf_("  Clockwise triangle prior to edge flip (bottom).\n");
            }
...
/*
            if (counterclockwise(rightpoint, farpoint, leftpoint) < 0.0) {
              printf_("Internal error in insertsite():\n");
              printf_("  Clockwise triangle prior to edge flip (top).\n");
            }
*/
            if (counterclockwise(farpoint, leftpoint, insertpoint) < 0.0) {
              printf_("Internal error in insertsite():\n");
              printf_("  Clockwise triangle after edge flip (left).\n");
            }
            if (counterclockwise(insertpoint, rightpoint, farpoint) < 0.0) {
              printf_("Internal error in insertsite():\n");
              printf_("  Clockwise triangle after edge flip (right).\n");
            }
          }
#endif /* SELF_CHECK */
          if (verbose > 2) {
  ...
  )
{
  ...
  if (verbose > 2) {
    printf_("  Triangulating interior polygon at edge\n");
    printf_("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasepoint[0],
  ...
  if (verbose > 2) {
    printf_("    Connecting edge to (%.12g, %.12g)\n", bestpoint[0],
           bestpoint[1]);
  }
  if (bestnumber > 1) {
  ...
  )
{
  ...
  while (!triedgeequal(*deltri, countingtri)) {
    ...
    if (countingtri.tri == dummytri) {
      printf_("Internal error in deletesite():\n");
      printf_("  Attempt to delete boundary point.\n");
      internal_error();
    }
#endif /* SELF_CHECK */
    edgecount++;
...
/*  vertex, which is represented by 0 and which represents nothing.       */
/*  The bounding triangles are linked in a circular fan about this 0      */
/*  vertex, and the edges on the convex hull of the triangulation appear     */
/*  opposite the 0 vertex.  You might find it easiest to imagine that     */
/*  the 0 vertex is a point in 3D space behind the center of the          */
/*  triangulation, and that the bounding triangles form a sort of cone.      */
/*                                                                           */
/*  This bounding box makes it easy to represent degenerate cases.  For      */
/*  instance, the triangulation of two vertices is a single edge.  This edge */
/*  is represented by two bounding box triangles, one on each "side" of the  */
/*  edge.  These triangles are also linked together in a fan about the 0  */
/*  vertex.                                                                  */
/*                                                                           */
...
/*  changed from 0 to a real point.                                       */
/*                                                                           */
  ...
  )
{
  ...
  /* Apex is intentionally left 0. */
  if (verbose > 2) {
    printf_("  Creating base bounding ");
  ...
  while (1) {
    ...
    if (!leftfinished) {
      ...
      /* If nextapex is 0, then no vertex would be exposed; the */
  ...
  )
{
  ...
  if (verbose > 2) {
    printf_("  Triangulating %d points.\n", vertices);
  }
  if (vertices == 2) {
    ...
    /* The apex is intentionally left 0. */
    ...
    if (area == 0.0) {
      ...
      /* All apices are intentionally left 0. */
      bond(midtri, tri1);
      bond(tri2, tri3);
      lnextself(midtri);
      lprevself(tri1);
      ...
      /* Apices of tri1, tri2, and tri3 are left 0. */
      if (area > 0.0) {
    ...
    divconqrecurse(&sortarray[divider], vertices - divider, 1 - axis,
                   &innerright, farright);
    if (verbose > 1) {
      printf_("  Joining triangulations with %d and %d vertices.\n", divider,
             vertices - divider);
    }
  ...
  )
{
  ...
  if (verbose) {
    printf_("  Removing ghost triangles.\n");
  }
...
long divconqdelaunay()
{
  ...
  sortarray = (point *) _malloc(inpoints * sizeof(point));
  if (sortarray == (point *) 0) {
    error( "Out of memory.\n");
  }
...
void boundingbox()
{
  ...
  if (verbose) {
    printf_("  Creating triangular bounding box.\n");
  }
  ...
  infpoint1 = (point) _malloc(points.itembytes);
  infpoint2 = (point) _malloc(points.itembytes);
  infpoint3 = (point) _malloc(points.itembytes);
  if ((infpoint1 == (point) 0) || (infpoint2 == (point) 0)
      || (infpoint3 == (point) 0)) {
    error( "Out of memory.\n");
  }
...
long removebox()
{
  ...
  if (verbose) {
    printf_("  Removing triangular bounding box.\n");
  }
  /* Find a boundary triangle. */
...
long incrementaldelaunay()
{
  ...
  boundingbox();
  if (verbose) {
    printf_("  Incrementally inserting points.\n");
  }
  traversalinit(&points);
  pointloop = pointtraverse();
  i = 1;
  while (pointloop != (point) 0) {
    /* Find a boundary triangle to search from. */
    starttri.tri = (triangle *) 0;
    if (insertsite(pointloop, &starttri, (struct edge *) 0, 0, 0) ==
        DUPLICATEPOINT) {
      if (!quiet) {
        printf_(
"Warning:  A duplicate point at (%.12g, %.12g) appeared and was ignored.\n",
  ...
  )
{
  ...
  *eventheap = (struct event **) _malloc(maxevents * sizeof(struct event *));
  if (*eventheap == (struct event **) 0) {
    error( "Out of memory.\n");
  }
  *events = (struct event *) _malloc(maxevents * sizeof(struct event));
  if (*events == (struct event *) 0) {
    error( "Out of memory.\n");
  }
  traversalinit(&points);
  for (i = 0; i < inpoints; i++) {
  ...
  )
{
  ...
  if (splaytree == (struct splaynode *) 0) {
    return (struct splaynode *) 0;
  }
  ...
  if (checkpoint == splaytree->keydest) {
    ...
    if (checkpoint != child->keydest) {
      ...
      if (child == (struct splaynode *) 0) {
        if (rightofroot) {
          splaytree->rchild = (struct splaynode *) 0;
        } else {
          splaytree->lchild = (struct splaynode *) 0;
        }
        return splaytree;
      }
    }
    ...
    if (grandchild == (struct splaynode *) 0) {
      if (rightofroot) {
    ...
    if (lefttree == (struct splaynode *) 0) {
      return righttree;
    } else if (righttree == (struct splaynode *) 0) {
      return lefttree;
    } else if (lefttree->rchild == (struct splaynode *) 0) {
      lefttree->rchild = righttree->lchild;
      righttree->lchild = lefttree;
      return righttree;
    } else if (righttree->lchild == (struct splaynode *) 0) {
...
/*      printf_("Holy Toledo!!!\n"); */
      ...
      while (leftright->rchild != (struct splaynode *) 0) {
  ...
  )
{
  ...
  if (splayroot == (struct splaynode *) 0) {
    newsplaynode->lchild = (struct splaynode *) 0;
    newsplaynode->rchild = (struct splaynode *) 0;
  } else if (rightofhyperbola(&splayroot->keyedge, searchpoint)) {
    newsplaynode->lchild = splayroot;
    newsplaynode->rchild = splayroot->rchild;
    splayroot->rchild = (struct splaynode *) 0;
  } else {
    newsplaynode->lchild = splayroot->lchild;
    newsplaynode->rchild = splayroot;
    splayroot->lchild = (struct splaynode *) 0;
  }
  return newsplaynode;
}
...
long sweeplinedelaunay()
{
  ...
  poolinit(&splaynodes, sizeof(struct splaynode), SPLAYNODEPERBLOCK, POINTER,
           0);
  splayroot = (struct splaynode *) 0;

  if (verbose) {
    printf_("  Placing points in event heap.\n");
  }
  ...
  do {
    if (heapsize == 0) {
      error( "Input points are all identical.\n");
    }
...
#else /* not REDUCED */
  if (!quiet) {
    printf_("Constructing Delaunay triangulation ");
    if (incremental) {
      printf_("by incremental method.\n");
    } else if (sweepline) {
      printf_("by sweepline method.\n");
    } else {
      printf_("by divide-and-conquer method.\n");
    }
  }
  if (incremental) {
...

{
  ...
  incorners = corners;
  if (incorners < 3) {
    error( "Triangles must have at least 3 points.\n");
  }
  eextras = attribs;
#else /* not TRILIBRARY */
  ...
  if (*stringptr == '\0') {
    ...
    if (incorners < 3) {
      error( "Triangles in %s must have at least 3 points.\n",
             elefilename);
    }
  }
  stringptr = findfield(stringptr);
  ...
  if (poly) {
    ...
    segmentmarkers = segmentmarkerlist != (int *) 0;
#else /* not TRILIBRARY */
  ...
  if (vararea) {
    ...
    if (areafile == (FILE *) 0) {
      error( "Error:  Cannot access file %s.\n", areafilename);
    }
  ...
  if (!quiet) {
    printf_("Reconstructing mesh.\n");
  }
  /* Allocate a temporary array that maps each point to some adjacent  */
  /*   triangle.  I took care to allocate all the permanent memory for */
  /*   triangles and shell edges first.                                */
  vertexarray = (triangle *) _malloc(points.items * sizeof(triangle));
  if (vertexarray == (triangle *) 0) {
    error( "Out of memory.\n");
  }
  ...
  while (triangleloop.tri != (triangle *) 0) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < firstnumber) || (corner[j] >= firstnumber + inpoints)) {
        error( "Triangle %d has an invalid vertex index.\n",
               elementnumber);
      }
    }
#else /* not TRILIBRARY */
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        error( "Triangle %d is missing point %d in %s.\n",
  ...
  if (poly) {
    if (verbose) {
      printf_("  Marking segments in triangulation.\n");
    }
    ...
    while (shelleloop.sh != (shelle *) 0) {
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < firstnumber) || (end[j] >= firstnumber + inpoints)) {
          error("Segment %d has an invalid vertex index.\n", 
                 segmentnumber);
        }
      }
  ...
  )
{
  ...
  while (leftflag) {
    ...
    if (searchtri->tri == dummytri) {
      printf_("Internal error in finddirection() turning left: "
  ...
  while (rightflag) {
    ...
    if (searchtri->tri == dummytri) {
      printf_("Internal error in finddirection() turning right: "
              "Unable to find a triangle leading from "
              "(%.16g, %.16g) to (%.16g, %.16g).\n",
  ...
  )
{
  ...
  denom = ty * ex - tx * ey;
  if (denom == 0.0) {
    printf_("Internal error in segmentintersection():");
    printf_("  Attempt to find intersection of parallel segments.\n");
    internal_error();
  }
  ...
  if (verbose > 1) {
    printf_(
    "  Splitting edge (%.16g, %.16g) (%.16g, %.16g) at (%.16g, %.16g).\n",
  ...
  if (success != SUCCESSFULPOINT) {
    printf_("Internal error in segmentintersection():\n");
    printf_("  Failure to split a segment.\n");
    internal_error();
  }
  if (steinerleft > 0) {
    steinerleft--;
  ...
  if ((leftpoint[0] == endpoint1[0]) && (leftpoint[1] == endpoint1[1])) {
    ...
    printf_("Internal error in segmentintersection():\n");
    printf_("  Topological inconsistency after splitting a segment.\n");
    internal_error();
  }
  ...
  )
{
  ...
  if (verbose > 2) {
    printf_("Forcing segment into triangulation by recursive splitting:\n");
    printf_("  (%.16g, %.16g) (%.16g, %.16g)\n", endpoint1[0], endpoint1[1],
  ...
  /* Find a boundary triangle to search from. */
  searchtri1.tri = (triangle *) 0;
  /* Attempt to insert the new point. */
  success = insertsite(newpoint, &searchtri1, (struct edge *) 0, 0, 0);
  if (success == DUPLICATEPOINT) {
    if (verbose > 2) {
      printf_("  Segment intersects existing point (%.16g, %.16g).\n",
    ...
    if (success == VIOLATINGPOINT) {
      if (verbose > 2) {
        printf_("  Two segments intersect at (%.16g, %.16g).\n",
               newpoint[0], newpoint[1]);
      }
      ...
      if (success != SUCCESSFULPOINT) {
        printf_("Internal error in conformingedge():\n");
        printf_("  Failure to split a segment.\n");
        internal_error();
      }
    }
  ...
  )
{
  ...
  if (verbose > 1) {
    printf_("  Connecting (%.16g, %.16g) to (%.16g, %.16g).\n",
  ...
  if (checkpoint != endpoint1) {
    ...
    if (locate(endpoint1, &searchtri1) != ONVERTEX) {
      ...
      printf_("  (%.16g, %.16g) in triangulation.\n",
             endpoint1[0], endpoint1[1]);
      internal_error();
    }
  }
...

{
  ...
  if (poly) {
    if (!quiet) {
      printf_("Inserting segments into Delaunay triangulation.\n");
    }
#ifdef TRILIBRARY
    ...
    if (segments > 0) {
      if (verbose) {
        printf_("  Inserting PSLG segments.\n");
      }
      makepointmap();
    }
    ...
    for (i = 1; i <= segments; i++) {
      ...
      if ((end1 < firstnumber) || (end1 >= firstnumber + inpoints)) {
        if (!quiet) {
          printf_("Warning:  Invalid first endpoint of segment %d in %s.\n", i,
                 polyfilename);
        }
      } else if ((end2 < firstnumber) || (end2 >= firstnumber + inpoints)) {
        if (!quiet) {
          printf_("Warning:  Invalid second endpoint of segment %d in %s.\n", i,
                 polyfilename);
        }
      } else {
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          if (!quiet) {
            printf_("Warning:  Endpoints of segment %d are coincident in %s.\n",
                   i, polyfilename);
          }
        } else {
  ...
  if (convex || !poly) {
    ...
    if (verbose) {
      printf_("  Enclosing convex hull with segments.\n");
    }
    markhull();
  }
  return segments;
...
void infecthull()
{
  ...
  if (verbose) {
    printf_("  Marking concavities (external triangles) for elimination.\n");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = dummytri;
  hulltri.orient = 0;
...
void plague()
{
  ...
  if (verbose) {
    printf_("  Marking neighbors of marked triangles.\n");
  }
  /* Loop through all the infected triangles, spreading the virus to */
  /*   their neighbors, then to their neighbors' neighbors.          */
  traversalinit(&viri);
  virusloop = (triangle **) traverse(&viri);
  while (virusloop != (triangle **) 0) {
    ...
    if (verbose > 2) {
      ...
      printf_("    Checking (%.16g, %.16g) (%.16g, %.16g) (%.16g, %.16g)\n",
      ...
      if ((neighbor.tri == dummytri) || infected(neighbor)) {
        ...
        if (neighborshelle.sh == dummysh) {
          ...
          if (verbose > 2) {
            ...
            printf_(
              "    Marking (%.16g, %.16g) (%.16g, %.16g) (%.16g, %.16g)\n",
                   deadorg[0], deadorg[1], deaddest[0], deaddest[1],
                   deadapex[0], deadapex[1]);
          }
          infect(neighbor);
  ...
  if (verbose) {
    printf_("  Deleting marked triangles.\n");
  }
  traversalinit(&viri);
  virusloop = (triangle **) traverse(&viri);
  while (virusloop != (triangle **) 0) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if (testpoint != (point) 0) {
        ...
        if (killorg) {
          if (verbose > 1) {
            printf_("    Deleting point (%.16g, %.16g)\n",
  ...
  )
{
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if ((neighbor.tri != dummytri) && !infected(neighbor)
        ...
        if (verbose > 2) {
          ...
          printf_("    Marking (%.16g, %.16g) (%.16g, %.16g) (%.16g, %.16g)\n",
                 regionorg[0], regionorg[1], regiondest[0], regiondest[1],
                 regionapex[0], regionapex[1]);
        }
        /* Infect the neighbor. */
  ...
  if (verbose > 1) {
    printf_("  Unmarking marked triangles.\n");
  }
  traversalinit(&viri);
  virusloop = (triangle **) traverse(&viri);
  while (virusloop != (triangle **) 0) {
  ...
  )
{
  ...
  if (!(quiet || (noholes && convex))) {
    printf_("Removing unwanted triangles.\n");
    if (verbose && (holes > 0)) {
      printf_("  Marking holes for elimination.\n");
    }
  }

  if (regions > 0) {
    /* Allocate storage for the triangles in which region points fall. */
    regiontris = (struct triedge *) _malloc(regions * sizeof(struct triedge));
    if (regiontris == (struct triedge *) 0) {
      error( "Out of memory.\n");
    }
  }
  ...
  if (regions > 0) {
    if (!quiet) {
      if (regionattrib) {
        if (vararea) {
          printf_("Spreading regional attributes and area constraints.\n");
        } else {
          printf_("Spreading regional attributes.\n");
        }
      } else { 
        printf_("Spreading regional area constraints.\n");
      }
    }
    ...
    for (i = 0; i < regions; i++) {
      if (regiontris[i].tri != dummytri) {
        ...
        if (regiontris[i].tri[3] != (triangle) 0) {
...
char const *precision_error_message()
{
  char const *s =
  ...
  )
{
  ...
  while ((badsegments.items > 0) && (steinerleft != 0)) {
    ...
    while ((encloop != (struct edge *) 0) && (steinerleft != 0)) {
      ...
      if ((success != SUCCESSFULPOINT) && (success != ENCROACHINGPOINT)) {
        printf_("Internal error in repairencs():\n");
        printf_("  Failure to split a segment.\n");
        internal_error();
      }
      if (steinerleft > 0) {
...
void tallyfaces()
{
  ...
  if (verbose) {
    printf_("  Making a list of bad triangles.\n");
  }
  traversalinit(&triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse();
  ...
  )
{
  ...
  if ((borg == badtri->faceorg) && (bdest == badtri->facedest) &&
    ...
    if (verbose > 1) {
      printf_("  Splitting this triangle at its circumcenter:\n");
      printf_("    (%.16g, %.16g) (%.16g, %.16g) (%.16g, %.16g)\n", borg[0],
    ...
    if (((newpoint[0] == borg[0]) && (newpoint[1] == borg[1]))
      ...
      if (!quiet) {
        printf_("Warning:  New point (%.16g, %.16g) falls on existing vertex.\n"
               , newpoint[0], newpoint[1]);
        errorflag = 1;
      }
      pointdealloc(newpoint);
      ...
      if (success == SUCCESSFULPOINT) {
        ...
        if (!quiet) {
          printf_(
            "Warning:  New point (%.16g, %.16g) falls on existing vertex.\n"
                 , newpoint[0], newpoint[1]);
          errorflag = 1;
        }
        pointdealloc(newpoint);
    ...
    if (errorflag) {
      if (verbose) {
        printf_("  The new point is at the circumcenter of triangle\n");
        printf_("    (%.16g, %.16g) (%.16g, %.16g) (%.16g, %.16g)\n",
               borg[0], borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
      }
      error( "This probably means that I am trying to refine triangles\n"
...
void enforcequality()
{
  ...
  if ((minangle > 0.0) || vararea || fixedarea) {
    ...
    for (i = 0; i < 64; i++) {
      queuefront[i] = (struct badface *) 0;
      queuetail[i] = &queuefront[i];
    }
    /* Test all triangles to see if they're bad. */
    tallyfaces();
    if (verbose) {
      printf_("  Splitting bad triangles.\n");
    }

    uint ITERATION_MAX = 500000U;
    uint iteration_count = 0;
  ...
  if (!quiet && (badsegments.items > 0) && (steinerleft == 0)) {
    ...
    if (badsegments.items == 1) {
      printf_("  an encroached segment, and therefore might not be truly\n");
    } else {
      printf_("  %ld encroached segments, and therefore might not be truly\n",
             badsegments.items);
    }
    printf_("  Delaunay.  If the Delaunay property is important to you,\n");
    printf_("  try increasing the number of Steiner points (controlled by\n");
    printf_("  the -S switch) slightly and try again.\n\n");
  }
}
...
void highorder()
{
  ...
  if (!quiet) {
    printf_("Adding vertices for second-order triangles.\n");
  }
...
char *infilename;
{
  ...
  do {
    ...
    if (result == (char *) 0) {
      error( "Error:  Unexpected end of file in %s.\n", infilename);
    }
...
FILE **polyfile;
{
  ...
  if (inpoints < 3) {
    error( "Input must have at least three input points.\n");
  }
  if (mesh_dim != 2) {
...
int *regions;
{
  ...
  *holes = (int) strtol (stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (REAL *) _malloc(2 * *holes * sizeof(REAL));
    *hlist = holelist;
    if (holelist == (REAL *) 0) {
      error( "Out of memory.\n");
    }
  ...
  if ((regionattrib || vararea) && !refine) {
    ...
    *regions = (int) strtol (stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (REAL *) _malloc(4 * *regions * sizeof(REAL));
      *rlist = regionlist;
      if (regionlist == (REAL *) 0) {
        error( "Out of memory.\n");
      }
      index = 0;
      for (i = 0; i < *regions; i++) {
...

{
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing points.\n");
  }

  if (points.items == 0) {
    error( "warning: no points\n" );
  }

  /* Allocate memory for output points if necessary. */
  if (*pointlist == (REAL *) 0) {
    if (points.items != 0) {
      *pointlist = (REAL *) _malloc(points.items * 2 * sizeof(REAL));
    }
  }
  /* Allocate memory for output point attributes if necessary. */
  if ((nextras > 0) && (*pointattriblist == (REAL *) 0)) {
    if (points.items != 0) {
      *pointattriblist =
        (REAL *) _malloc(points.items * nextras * sizeof(REAL));
    }
  }
  /* Allocate memory for output point markers if necessary. */
  if (!nobound && (*pointmarkerlist == (int *) 0)) {
    if (points.items != 0) {
      *pointmarkerlist = (int *) _malloc(points.items * sizeof(int));
    }
  }
  plist = *pointlist;
  ...
  while (pointloop != (point) 0) {
    ...
    fprintf_(outfile, "%4d    %.17g  %.17g", pointnumber, pointloop[0],
            pointloop[1]);
    for (i = 0; i < nextras; i++) {
      /* Write an attribute. */
      fprintf_(outfile, "  %.17g", pointloop[i + 2]);
    }
    if (nobound) {
      fprintf_(outfile, "\n");
    } else {
...

{
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing triangles.\n");
  }
  if (triangles.items == 0) {
    error("warning: no faces\n");
  }

  /* Allocate memory for output triangles if necessary. */
  if (*trianglelist == (int *) 0) {
    if (triangles.items != 0) {
      *trianglelist = (int *)
        _malloc(triangles.items * ((order + 1) * (order + 2) / 2) *
                sizeof(int));
    }
  }
  /* Allocate memory for output triangle attributes if necessary. */
  if ((eextras > 0) && (*triangleattriblist == (REAL *) 0)) {
    if (triangles.items != 0) {
      *triangleattriblist = (REAL *) _malloc(triangles.items * eextras *
                                             sizeof(REAL));
    }
  }
  tlist = *trianglelist;
  ...
  while (triangleloop.tri != (triangle *) 0) {
    ...
    if (order == 1) {
      ...
      fprintf_(outfile, "%4d    %4d  %4d  %4d", elementnumber,
      ...
      fprintf_(outfile, "%4d    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber,
...
#else /* not TRILIBRARY */
    for (i = 0; i < eextras; i++) {
      fprintf_(outfile, "  %.17g", elemattribute(triangleloop, i));
    }
    fprintf_(outfile, "\n");
#endif /* not TRILIBRARY */
...

{
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing segments.\n");
  }
  if (shelles.items == 0) {
    error("warning: no segments\n");
  }
  /* Allocate memory for output segments if necessary. */
  if (*segmentlist == (int *) 0) {
    if (shelles.items != 0) {
      *segmentlist = (int *) _malloc(shelles.items * 2 * sizeof(int));
    }
  }
  /* Allocate memory for output segment markers if necessary. */
  if (!nobound && (*segmentmarkerlist == (int *) 0)) {
    if (shelles.items != 0) {
      *segmentmarkerlist = (int *) _malloc(shelles.items * sizeof(int));
    }
  }
  slist = *segmentlist;
  ...
  while (shelleloop.sh != (shelle *) 0) {
    ...
    if (nobound) {
      fprintf_(outfile, "%4d    %4d  %4d\n", shellenumber,
              pointmark(endpoint1), pointmark(endpoint2));
    } else {
      fprintf_(outfile, "%4d    %4d  %4d    %4d\n", shellenumber,
  ...
  if (regions > 0) {
    ...
    for (i = 0; i < regions; i++) {
      ...
      fprintf_(outfile, "%4d   %.17g  %.17g  %.17g  %.17g\n", firstnumber + i,
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing edges.\n");
  }
  /* Allocate memory for edges if necessary. */
  if (*edgelist == (int *) 0) {
    *edgelist = (int *) _malloc(edges * 2 * sizeof(int));
    if (*edgelist == (int *) 0) {
      error( "Out of memory.\n");
    }
  }
  /* Allocate memory for edge markers if necessary. */
  if (!nobound && (*edgemarkerlist == (int *) 0)) {
    *edgemarkerlist = (int *) _malloc(edges * sizeof(int));
    if (*edgemarkerlist == (int *) 0) {
      error( "Out of memory.\n");
    }
  }
  elist = *edgelist;
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing Voronoi vertices.\n");
  }
  /* Allocate memory for Voronoi vertices if necessary. */
  if (*vpointlist == (REAL *) 0) {
    *vpointlist = (REAL *) _malloc(triangles.items * 2 * sizeof(REAL));
    if (*vpointlist == (REAL *) 0) {
      error( "Out of memory.\n");
    }
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (REAL *) 0) {
    *vpointattriblist = (REAL *) _malloc(triangles.items * nextras *
                                        sizeof(REAL));
    if (*vpointattriblist == (REAL *) 0) {
      error( "Out of memory.\n");
    }
  }
  *vpointmarkerlist = (int *) 0;
  plist = *vpointlist;
  ...
  while (triangleloop.tri != (triangle *) 0) {
    ...
    fprintf_(outfile, "%4d    %.17g  %.17g", vnodenumber, circumcenter[0],
            circumcenter[1]);
    for (i = 2; i < 2 + nextras; i++) {
      /* Interpolate the point attributes at the circumcenter. */
      fprintf_(outfile, "  %.17g", torg[i] + xi * (tdest[i] - torg[i])
                                         + eta * (tapex[i] - torg[i]));
    }
    fprintf_(outfile, "\n");
#endif /* not TRILIBRARY */
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing Voronoi edges.\n");
  }
  /* Allocate memory for output Voronoi edges if necessary. */
  if (*vedgelist == (int *) 0) {
    *vedgelist = (int *) _malloc(edges * 2 * sizeof(int));
    if (*vedgelist == (int *) 0) {
      error( "Out of memory.\n");
    }
  }
  *vedgemarkerlist = (int *) 0;
  /* Allocate memory for output Voronoi norms if necessary. */
  if (*vnormlist == (REAL *) 0) {
    *vnormlist = (REAL *) _malloc(edges * 2 * sizeof(REAL));
    if (*vnormlist == (REAL *) 0) {
      error( "Out of memory.\n");
    }
  }
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == dummytri)) {
        ...
        if (trisym.tri == dummytri) {
          ...
          fprintf_(outfile, "%4d   %d  %d   %.17g  %.17g\n", vedgenumber,
...
#ifdef TRILIBRARY
  if (!quiet) {
    printf_("Writing neighbors.\n");
  }
  /* Allocate memory for neighbors if necessary. */
  if (*neighborlist == (int *) 0) {
    *neighborlist = (int *) _malloc(triangles.items * 3 * sizeof(int));
    if (*neighborlist == (int *) 0) {
      error( "Out of memory.\n");
    }
  }
...
void quality_statistics()
{
  ...
  printf_("Mesh quality statistics:\n\n");
  radconst = PI / 18.0;
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
  ...
  printf_("  Smallest area: %16.5g   |  Largest area: %16.5g\n",
         smallestarea, biggestarea);
  printf_("  Shortest edge: %16.5g   |  Longest edge: %16.5g\n",
         shortest, longest);
  printf_("  Shortest altitude: %12.5g   |  Largest aspect ratio: %8.5g\n\n",
         minaltitude, worstaspect);
  printf_("  Aspect ratio histogram:\n");
  printf_("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
         ratiotable[0], aspecttable[0], ratiotable[7], ratiotable[8],
         aspecttable[8]);
  for (i = 1; i < 7; i++) {
    printf_("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
           ratiotable[i - 1], ratiotable[i], aspecttable[i],
           ratiotable[i + 7], ratiotable[i + 8], aspecttable[i + 8]);
  }
  printf_("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
         ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
         aspecttable[15]);
  printf_(
"  (Triangle aspect ratio is longest edge divided by shortest altitude)\n\n");
  printf_("  Smallest angle: %15.5g   |  Largest angle: %15.5g\n\n",
         smallestangle, biggestangle);
  printf_("  Angle histogram:\n");
  for (i = 0; i < 9; i++) {
    printf_("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
...
{
  printf_("\nStatistics:\n\n");
  printf_("  Input points: %d\n", inpoints);
  if (refine) {
    printf_("  Input triangles: %d\n", inelements);
  }
  if (poly) {
    printf_("  Input segments: %d\n", insegments);
    if (!refine) {
      printf_("  Input holes: %d\n", holes);
    }
  }

  printf_("\n  Mesh points: %ld\n", points.items);
  printf_("  Mesh triangles: %ld\n", triangles.items);
  printf_("  Mesh edges: %ld\n", edges);
  if (poly || refine) {
    printf_("  Mesh boundary edges: %ld\n", hullsize);
    printf_("  Mesh segments: %ld\n\n", shelles.items);
  } else {
    printf_("  Mesh convex hull edges: %ld\n\n", hullsize);
  }
  if (verbose) {
    quality_statistics();
    printf_("Memory allocation statistics:\n\n");
    printf_("  Maximum number of points: %ld\n", points.maxitems);
    printf_("  Maximum number of triangles: %ld\n", triangles.maxitems);
    if (shelles.maxitems > 0) {
      printf_("  Maximum number of segments: %ld\n", shelles.maxitems);
    }
    if (viri.maxitems > 0) {
      printf_("  Maximum number of viri: %ld\n", viri.maxitems);
    }
    if (badsegments.maxitems > 0) {
      printf_("  Maximum number of encroached segments: %ld\n",
             badsegments.maxitems);
    }
    if (badtriangles.maxitems > 0) {
      printf_("  Maximum number of bad triangles: %ld\n",
             badtriangles.maxitems);
    }
    if (splaynodes.maxitems > 0) {
      printf_("  Maximum number of splay tree nodes: %ld\n",
             splaynodes.maxitems);
    }
    printf_("  Approximate heap memory use (bytes): %ld\n\n",
    ...
    printf_("Algorithmic statistics:\n\n");
    printf_("  Number of incircle tests: %ld\n", incirclecount);
    printf_("  Number of orientation tests: %ld\n", counterclockcount);
    if (hyperbolacount > 0) {
      printf_("  Number of right-of-hyperbola tests: %ld\n",
             hyperbolacount);
    }
    if (circumcentercount > 0) {
      printf_("  Number of circumcenter computations: %ld\n",
             circumcentercount);
    }
    if (circletopcount > 0) {
      printf_("  Number of circle top computations: %ld\n",
             circletopcount);
    }
    printf_("\n");
  }
  ...
  struct triangulateio *vorout )
{
  ...
  if (!quiet) {
    gettimeofday(&tv2, &tz);
    if (refine) {
      printf_("Mesh reconstruction");
    } else {
      printf_("Delaunay");
    }
    printf_(" milliseconds:  %ld\n", 1000l * (tv2.tv_sec - tv1.tv_sec)
  ...
  if (!quiet) {
    ...
    if (useshelles && !refine) {
      printf_("Segment milliseconds:  %ld\n",
...
#ifndef CDT_ONLY
    if (quality) {
      printf_("Quality milliseconds:  %ld\n",
  ...
  if (vorout != (struct triangulateio *) 0) {
  ...
  if (nonodewritten || (noiterationnum && readnodefile)) {
    if (!quiet) {
#ifdef TRILIBRARY
      printf_("NOT writing points.\n");
#else /* not TRILIBRARY */
      printf_("NOT writing a .node file.\n");
#endif /* not TRILIBRARY */
    }
  ...
  if (noelewritten) {
    if (!quiet) {
#ifdef TRILIBRARY
      printf_("NOT writing triangles.\n");
#else /* not TRILIBRARY */
      printf_("NOT writing an .ele file.\n");
#endif /* not TRILIBRARY */
    }
  } else {
#ifdef TRILIBRARY
  ...
  if (poly || convex) {
    ...
    if (nopolywritten || noiterationnum) {
      if (!quiet) {
#ifdef TRILIBRARY
        printf_("NOT writing segments.\n");
#else /* not TRILIBRARY */
        printf_("NOT writing a .poly file.\n");
#endif /* not TRILIBRARY */
      }
    } else {
#ifdef TRILIBRARY
  ...
  if (!quiet) {
    ...
    printf_("Total running milliseconds:  %ld\n",
