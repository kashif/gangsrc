/* GANG Software
 * WIDGET/gtk/widgetc/gtk_filesel2.c
...
static void gtk_file_selection2_init          (GtkFileSelection2      *filesel);
...
static void gtk_file_selection2_populate      (GtkFileSelection2      *fs,
					      gchar                 *rel_path,
					      gint                   try_complete);
static void gtk_file_selection2_abort         (GtkFileSelection2      *fs);

static void gtk_file_selection2_update_history_menu (GtkFileSelection2       *fs,
						    gchar                  *current_dir);
...
{
  static GtkType file_selection2_type = 0;

  if (!file_selection2_type)
    {
	...
	/* reserved_2 */ NULL,
      ...
      file_selection2_type = gtk_type_unique (GTK_TYPE_VBOX, &filesel_info);
    }

  return file_selection2_type;
}

static void
gtk_file_selection2_class_init (GtkFileSelection2Class *class)
{
...
static void
gtk_file_selection2_init (GtkFileSelection2 *filesel)
{
  ...
  gtk_file_selection2_show_fileop_buttons(filesel);
  ...
  gtk_signal_connect (GTK_OBJECT (filesel->selection_entry), "key_press_event",
		      (GtkSignalFunc) gtk_file_selection2_key_press, filesel);
  ...
  if (!cmpl_state_okay (filesel->cmpl_state))
    ...
    {
      gtk_file_selection2_populate (filesel, "", FALSE);
    }
...
GtkWidget*
gtk_file_selection2_new ()
{
  GtkFileSelection2 *filesel;

  filesel = gtk_type_new (GTK_TYPE_FILE_SELECTION2);
...
void
gtk_file_selection2_show_fileop_buttons (GtkFileSelection2 *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_FILE_SELECTION2 (filesel));
...
void       
gtk_file_selection2_hide_fileop_buttons (GtkFileSelection2 *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_FILE_SELECTION2 (filesel));
...
void
gtk_file_selection2_set_filename (GtkFileSelection2 *filesel,
				 const gchar      *filename)
{
  ...
  gtk_file_selection2_populate (filesel, buf, FALSE);
...
gchar*
gtk_file_selection2_get_filename (GtkFileSelection2 *filesel)
{
  ...
  g_return_val_if_fail (GTK_IS_FILE_SELECTION2 (filesel), nothing);
...
void
gtk_file_selection2_complete (GtkFileSelection2 *filesel,
			     const gchar      *pattern)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_FILE_SELECTION2 (filesel));
  ...
  gtk_file_selection2_populate (filesel, (gchar*) pattern, TRUE);
}
...
gtk_file_selection2_destroy (GtkObject *object)
{
  ...
  filesel = GTK_FILE_SELECTION2 (object);
...
static void
gtk_file_selection2_fileop_error (GtkFileSelection2 *fs, gchar *error_message)
{
  GtkWidget *label;
  GtkWidget *vbox;
  GtkWidget *button;
  ...
  if ( (mkdir (full_path, 0755) < 0) ) 
    {
      ...
      gtk_file_selection2_fileop_error (fs, buf);
    }
  g_free (full_path);
  
  gtk_widget_destroy (fs->fileop_dialog);
  gtk_file_selection2_populate (fs, "", FALSE);
}
  ...
  if (event->keyval == GDK_Tab)
    {
      ...
      gtk_file_selection2_populate (fs, text, TRUE);
...
static void
gtk_file_selection2_history_callback (GtkWidget *widget, gpointer data)
{
  GtkFileSelection2 *fs = data;
  HistoryCallbackArg *callback_arg;
  GList *list;
  ...
  while (list) {
    ...
    if (callback_arg->menu_item == widget)
      {
	gtk_file_selection2_populate (fs, callback_arg->directory, FALSE);
	break;
      }
  ...
  for (i = dir_len; i >= 0; i--)
      ...
      if ( (current_dir[i] == '/') || (i == dir_len))
	{
	  ...
	  gtk_signal_connect (GTK_OBJECT (menu_item), "activate",
			      (GtkSignalFunc) gtk_file_selection2_history_callback,
			      (gpointer) fs);
			       ...
			       gpointer user_data)
{
  ...
  if (filename)
	...
	switch (bevent->type)
	  {
	  case GDK_2BUTTON_PRESS:
	    gtk_file_selection2_populate (fs, filename, FALSE);
	    break;
...
static void
gtk_file_selection2_populate (GtkFileSelection2 *fs,
			     gchar            *rel_path,
			     gint              try_complete)
{
  CompletionState *cmpl_state;
      ...
      if (cmpl_updated_text (cmpl_state)[0])
        {
	      ...
	      gchar* dir_name = g_strdup (cmpl_updated_text (cmpl_state));
              ...
              gtk_file_selection2_populate (fs, dir_name, TRUE);
  ...
  if (!did_recurse)
	...
	{
	  gtk_file_selection2_update_history_menu (fs, cmpl_reference_position (cmpl_state));
	}
